...
The code below is based on the information found at https://www.kaggle.com/jimthompson/using-the-boruta-package-to-determine-fe.  

This report determines what features may be relevant to modeling the **target** attribute for the case_8 data set. This analysis is based on the [**Boruta**](https://cran.r-project.org/web/packages/Boruta/index.html) package. In addition we demonstrate the [**caret**](https://cran.r-project.org/web/packages/caret/index.html) package functions to create data partitions and impute missing values.
...
Read in the case_8 data set, determine data types and perform simple pre-processing for numeric data.  
```{r}
# retrive sample data for analysis
train <- read.csv(("data/case_8.csv"),stringsAsFactors = FALSE)

###
# select random sample for analysis using caret createDataPartition() function
###
set.seed(123)
idx <- createDataPartition(train$target,p=0.05,list=FALSE)
sample.df <- train[idx,]
...
The following plot shows the relevance of each feature. Green color indicate the feature has been deemed relevance to the **target** attribute. Red color indicates the attribute is not relevant. Yellow attributes are tentative, may or may not be relevant. Increasing maxRuns parameter can help further refine the list of tentative attributes as relevant or not.  
